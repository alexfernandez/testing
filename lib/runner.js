...
var Runner = function(key, series)
{
	...
	self.runAll = function(callback)
	{
		runOne(function(error, result)
		{
			if (error)
			{
				console.error('Error in test %s: %s', key, error);
			}
			if (result)
			{
				console.log('Finished test %s: %s', key, result.getSummary());
			}
			if (finished)
			{
				return;
	...
	function runOne(callback)
	{
		...
		for (var key in series)
		{
			...
			if (!value)
			{
				return callback('Empty test value for ' + key);
			}
	 ...
	 */
	function getNext(key, testResult, callback)
	{
